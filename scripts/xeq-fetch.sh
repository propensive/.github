...
# propensive/xeq with the runner stubs. Applications shell out to it to package their executables
# rather than carrying a copy of their own; each repository pins the version and the script's
# SHA-256 in its own etc/xeq.tsv, so this script can be shared while the pin stays local.
#
