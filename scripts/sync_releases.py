#!/usr/bin/env python3
"""Synchronize ~/.ivy2/local with the jars a propensive repository publishes on GitHub Releases.

Invoked by `sync-deps.sh` (for every pin in etc/refs) and by `snapshot.sh` (for the jars it
has just staged). Each released jar embeds its POM and ivy.xml under
`META-INF/maven/<group>/<artifactId>/`, so the jar alone reconstructs the ivy2 layout coursier
expects:
...
Usage: sync_releases.py --repo owner/repo X.Y.Z        one release
       sync_releases.py --repo owner/repo               every X.Y.Z release of the repository
...
sync-deps.sh takes as its cue to build the snapshot locally.

Environment: GITHUB_TOKEN lifts the API rate limit; IVY_LOCAL overrides the destination;
SOUNDNESS_RELEASE_REPO is the older spelling of --repo.
"""
...
ASSET_NAME = re.compile(r"^(.+)-(\d+\.\d+\.\d+(?:-[0-9a-f]{12})?)\.jar$")
NO_SUCH_RELEASE = 3
...
def release_assets(release: dict) -> list[Asset]:
    """The library jars among a release's assets: those named `<artifactId>-<version>.jar`. A
    launcher release also carries executables and scripts, which do not match and are skipped."""
    assets: list[Asset] = []
    for asset in release.get("assets", []):
        ...
        assets.append(Asset(match.group(1), match.group(2), asset["browser_download_url"], digest))
    return assets


def release_by_tag(repo: str, tag: str) -> dict | None:
    """One release by its tag, or None when there is none (a 404)."""
    try:
        return api(f"https://api.github.com/repos/{repo}/releases/tags/{tag}")
...
def main(arguments: list[str]) -> None:
    ...
    if arguments and arguments[0] == "--staged":
        ...
        if release is None:
            fail(f"{repo} has no release tagged {tag}", NO_SUCH_RELEASE)
        assets = release_assets(release)
        log(f"syncing {len(assets)} jars from {repo} ({tag})")
    else:
        wanted = arguments[0] if arguments else None
        if wanted is not None and not VERSION_TAG.match(wanted):
            fail(f"version must be X.Y.Z (got '{wanted}'); a snapshot is synced by --tag")
        if wanted is not None:
            release = release_by_tag(repo, wanted)
            if release is None:
                fail(f"{repo} has no release {wanted}", NO_SUCH_RELEASE)
            selected = [release]
        else:
            selected = [release for release in releases(repo) if VERSION_TAG.match(release["tag_name"])]
